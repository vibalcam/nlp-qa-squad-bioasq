...
def main(args):
    ...
    # Make a copy of the first dev_dataset
    ...
    print(f'second train samples = {len(train_dataset)}')
    print(f'second dev samples = {len(dev_dataset)}')
    print()

    # Select model.
    model = _select_model(args)
    num_pretrained = model.load_pretrained_embeddings(
        vocabulary, args.embedding_path, args.second_embedding_path
    )
    ...
    if args.do_test:
        ...
        print()
        print(f'first predictions written to \'{original_output_path}\'')
